...
public class TOCLayerManager {
	...
	for (int i = 0; i < layersInTOC.getLayersCount(); i++) {
	    ...
	    if (layerName.equalsIgnoreCase(Preferences.REGIONES_LAYER_NAME)) {
    ...
    public boolean isManzanaLayerInEdition() {
	FLyrVect manzana = getLayerByName(Preferences.MANZANAS_LAYER_NAME);
	return manzana.isEditing();
    }

    public boolean isPrediosLayerInEdition() {
